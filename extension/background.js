...
async function probeContentScriptVersion(tabId) {
  try {
    const probe = await chrome.scripting.executeScript({
      target: { tabId },
      func: () => globalThis.__BB_CONTENT_SCRIPT_LOADED__ || ""
    });
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  // ========== 批量导入：获取单个视频的元数据和字幕 ==========
  if (message.type === "batch-fetch-video") {
    const bvid = String(message.bvid || "").trim();
    const targetPage = Number(message.page) || 0; // 0 = 第一页，>0 = 指定分P
    if (!bvid) {
      sendResponse({ ok: false, error: "缺少 BV 号" });
      return false;
    }
    ...
    (async () => {
      try {
        // 1. 获取视频元数据
        const meta = await fetchBiliVideoMetaByBvid(bvid);
        const pages = meta.pages || [];
        const allPages = pages.map(p => ({
          cid: String(p.cid || "").trim(),
          page: Number(p.page || 0),
          part: String(p.part || "").trim(),
          duration: Number(p.duration || 0)
        }));

        // 选择指定分P（targetPage 从1开始，数组从0开始）
        const pageIndex = targetPage > 0 ? targetPage - 1 : 0;
        const selectedPage = allPages[pageIndex] || allPages[0] || {
          cid: meta.defaultCid,
          page: 1,
          part: "",
          duration: meta.defaultDuration
        };
        const cid = String(selectedPage.cid || meta.defaultCid || "").trim();
        const aid = String(meta.aid || "").trim();

        if (!cid) {
          sendResponse({ ok: false, error: "无法获取视频 cid" });
          return;
        }

        // 2. 获取字幕列表
        const subtitleBundle = await fetchBiliSubtitleBundle({ bvid, cid, aid });
        const tracks = normalizeSubtitleTracks(subtitleBundle.tracks || []);

        if (!tracks.length) {
          sendResponse({ ok: false, error: "该视频无可用字幕" });
          return;
        }

        // 3. 选择中文字幕（优先）
        const selectedTrack = pickPreferredSubtitleTrack(tracks) || tracks[0];
        const subtitleBody = await fetchBiliSubtitleBody(selectedTrack.subtitleUrl);

        // 4. 构建返回数据
        const videoUrl = buildCanonicalVideoUrl(bvid, Number(selectedPage.page || 1));
        sendResponse({
          ok: true,
          meta: {
            bvid,
            ...
            url: videoUrl,
            pageIndex: Number(selectedPage.page || 1),
            pageTitle: String(selectedPage.part || "").trim(),
            subtitleLang: String(selectedTrack.lanDoc || selectedTrack.lan || "").trim()
          },
          pages: allPages, // 返回所有分P信息
  ...
  if (message.type === "open-reading-view-tab") {
    ...
    try {
      ...
      parsed.searchParams.set("bb_reader", "1");
